...
public class VendorPage {
	...
	public void AddresRegion(String strValue)
	{
		...
		Sync.waitForSeconds(Constants.WAIT_1);
		region.selectByVisibleText("NOE, Lower Austria");
		/*List <WebElement> elementCount = region.getOptions();
		 int iSize = elementCount.size();
		 System.out.println("Drop down count for region"+iSize);
		 for(int i =0; i<iSize ; i++)
		 {
			String sValue = elementCount.get(i).getText();
			if(sValue.equals(strValue))
...
//		 System.out.println(iSize);
	...
	public void AddresIndustryKey(String strValue)
	{
		...
		//		IndustryKey.selectByVisibleText("Y001, Trade/Serv/Transport");
		Sync.waitForSeconds(Constants.WAIT_1);
	...
	public void AddresCorporateGroup(String strValue)
	{
		...
		CorporateGroup.selectByVisibleText("PR-PACK, PR-Packaging materials");
		/*Sync.waitForSeconds(Constants.WAIT_1);
		...
		driver.findElement(By.xpath("//*[text()='Production location']")).click();
	}
